...
for(let i = 0; i < cuadrados.length; i++){
  if (cuadrados[i].classList.contains("piso")) {
    continue
  }

  cuadrados[i].onclick = function () {
    if (cuadrados[i].classList.contains("taken")) {
      return 
    }
    if (ganador) {
      return 
...
function ganaste() {
  for (let index = 0; index < combinacionesGanadoras.length; index++) {
    let jugadorAzulGana = true
    let jugadorVerdeGana = true
    const array = combinacionesGanadoras[index];
    for (let i = 0; i < array.length; i++) {
      const element = array[i];
      const cuadrado = cuadrados[element]
      if (!cuadrado.classList.contains("jugador-azul")) {
       jugadorAzulGana = false 
      }
      if (!cuadrado.classList.contains("jugador-verde")) {
        jugadorVerdeGana = false 
      }
    }
    if (jugadorAzulGana) {
      return "azul"
